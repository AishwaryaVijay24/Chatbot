...
async function callPredict()
{
  ...
  let prompt = {};
    ...
    if (prompt.messages.length > 1) {
      previousResponse = prompt.messages[1].content;
    }

    const hasContinuationCue = nextInput.startsWith("any other"); // Check for continuation cue

    if (hasContinuationCue && previousResponse) {
      // Construct a prompt that includes the previous response for context:
      prompt.messages = [
        { author: "user", content: previousResponse },
        { author: "user", content: nextInput },
        { author: "system", content: "Please provide specific details or elaborate on your question." }
      ];
    } else {
      // Construct prompt without explicit context:
      prompt.messages = [
        { author: "user", content: nextInput },
        { author: "system", content: "Please provide specific details or elaborate on your question." }
      ];
